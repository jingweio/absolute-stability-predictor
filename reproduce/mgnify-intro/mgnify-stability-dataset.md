...
- **"orphan" 是我们从数据推导的概念**（论文未用此词）：某 scaffold 只被 mutant 引用、自己没有 WT 标签行 —— 很可能其 WT 被过滤出训练集（<30% identity to test / 质量 / 落 val-test），**但作者侧其 domain 结构仍存在**（AF2 按 WT domain 折）。orphan 缺的是 **WT 的 ΔG label**，非结构。
...
- **训练结构来源**：论文用 **AlphaFold2 预测结构（+ PAE）**，**每序列一份结构**；mutant 用母体 WT backbone（Fig 3a："for a given backbone structure, WT and mutant sequences are evaluated independently"）。
...
## 7. 尚未解决 / 待核实
- **orphan（4,623, <1%）**：我们按 WT 行 fold，未折出其结构；作者侧结构应存在（按 WT domain 折）。若要补齐，需拿到 orphan 母体的 **WT 序列**（不在训练索引里），或用一条 mutant 当代理折。
- 作者发布的结构集是否**物理包含**每个 orphan scaffold 文件，未从数据坐实（本文 §4/§5 关于 orphan 结构的判断是**基于 Methods 描述 + `PDB_name` 语义的推断**）。
